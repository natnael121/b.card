...
import { X, Calendar as CalendarIcon, Clock, Check, AlertCircle, ChevronRight, User, Mail, Phone, Building, MessageSquare } from 'lucide-react';
...
export default function AppointmentForm({ card, onClose }: AppointmentFormProps) {
  ...
  const fetchAvailability = async () => {
    ...
    try {
      const calendarId = card.google_calendar_id || 'primary';
      
      // Get a fresh token for the owner
      const accessToken = await getFreshAccessToken(card.id);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const googleEvent = await createCalendarEvent(accessToken, calendarId, {
        ...
        visitorNotes: formData.visitor_notes,
        startTime: slotStart,
        endTime: slotEnd,
      });
  ...
  if (success) {
    return (
      <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center p-4 z-50 animate-fade-in">
        <div className="bg-neutral-900 rounded-3xl shadow-2xl max-w-md w-full p-8 border border-neutral-800 text-center">
          ...
          <h3 className="text-2xl font-bold text-white mb-2">Appointment Scheduled!</h3>
          <p className="text-neutral-400 mb-4 text-sm">
            Your appointment with <span className="font-semibold text-white">{card.full_name}</span> has been confirmed.
          </p>
          <div className="bg-neutral-800 rounded-xl p-4 text-xs text-neutral-300 font-medium">
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center p-4 z-50">
      <div className="bg-neutral-900 border border-neutral-800 rounded-3xl shadow-2xl max-w-md w-full overflow-hidden flex flex-col max-h-[90vh]">
        {/* Header */}
        <div className="p-6 border-b border-neutral-800 flex justify-between items-center bg-neutral-900 sticky top-0 z-10">
          <div>
            ...
            <p className="text-xs text-neutral-400 mt-1">With {card.full_name}</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 overflow-y-auto flex-1 space-y-6">
          {error && (
            <div className="bg-red-500 bg-opacity-10 border border-red-500 text-red-400 p-4 rounded-2xl flex items-start gap-3 text-sm">
          ...
          {step === 1 ? (
            <div className="space-y-6">
              {/* Date Selection */}
              <div>
                <label className="block text-xs font-semibold text-neutral-400 mb-3 uppercase tracking-wider">
                  Select Date
                </label>
                <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-thin scrollbar-thumb-neutral-800 scrollbar-track-transparent">
                  {dates.map((d, index) => {
                    ...
                    return (
                      <button
                        ...
                        className={`flex-shrink-0 px-4 py-3 rounded-2xl border text-center transition flex flex-col items-center min-w-[84px] ${
                          isSelected
              ...
              {/* Slots Selection */}
              <div>
                <label className="block text-xs font-semibold text-neutral-400 mb-3 uppercase tracking-wider flex items-center justify-between">
                  <span>Select Time (Business Hours)</span>
                  <span className="text-[10px] text-neutral-500 normal-case font-normal">Your local time</span>
                </label>
                ...
                {loadingSlots ? (
                  ...
                  <div className="text-center py-12 bg-neutral-800 bg-opacity-30 rounded-2xl border border-neutral-800">
                    <Clock size={24} className="mx-auto text-neutral-500 mb-2" />
                    <p className="text-sm text-neutral-400 font-medium">No slots available today</p>
                    <p className="text-xs text-neutral-500 mt-1">Please try selecting another date</p>
                  </div>
                ) : (
                    ...
                    {timeSlots.map((slot, index) => {
                      ...
                      return (
                        <button
                          ...
                          className={`py-3 px-2 rounded-xl text-center text-xs font-semibold border transition ${
                            isSelected
                              ? 'bg-white text-black border-white'
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2 bg-white text-black py-4 px-6 rounded-2xl hover:bg-neutral-200 transition font-bold disabled:opacity-50 disabled:cursor-not-allowed mt-4"
              >
                <span>Continue</span>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              {/* Selected Slot Recap */}
              <div className="bg-neutral-850 rounded-2xl p-4 border border-neutral-800 flex items-start gap-3">
                <CalendarIcon className="text-white mt-0.5" size={18} />
                <div>
                  <p className="text-[10px] text-neutral-400 font-bold uppercase tracking-wider">Selected Time</p>
                  <p className="text-xs text-white font-semibold mt-1">{getFriendlySlotDetails()}</p>
                  <button
                    type="button"
                    onClick={() => setStep(1)}
                    className="text-[11px] text-blue-400 hover:underline font-medium mt-1"
                  >
                    Change date/time
                  </button>
                </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full bg-neutral-850 border border-neutral-800 text-white rounded-2xl py-3.5 pl-12 pr-4 outline-none focus:border-neutral-600 transition text-sm"
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  <MessageSquare size={16} className="absolute left-4 top-4 text-neutral-500" />
                  <textarea
                    rows={3}
                    value={formData.visitor_notes}
                    onChange={(e) => setFormData({ ...formData, visitor_notes: e.target.value })}
                    placeholder="What would you like to discuss?"
                    className="w-full bg-neutral-850 border border-neutral-800 text-white rounded-2xl py-3.5 pl-12 pr-4 outline-none focus:border-neutral-600 transition text-sm resize-none"
                  />
                </div>
              </div>
              ...
              <div className="flex gap-3 pt-4 border-t border-neutral-800 mt-6">
                <button
                  type="button"
                  onClick={() => setStep(1)}
                  className="flex-1 py-4 border border-neutral-800 text-neutral-300 font-semibold rounded-2xl hover:bg-neutral-800 hover:text-white transition text-sm"
                >
                  Back
                </button>
                <button
                  type="submit"
                  disabled={submitting}
                  className="flex-1 bg-white text-black font-semibold rounded-2xl hover:bg-neutral-200 transition disabled:opacity-50 disabled:cursor-not-allowed text-sm"
                >
